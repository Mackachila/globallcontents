...
app.post('/register', express.urlencoded({ extended: true }), async (req, res) => {
  ...
  try {
    ...
    const insertInvitesQuery = `
      INSERT INTO invites (invited, inviter, firstrecharge, status)
...
app.post('/update-level/:level', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const checkTaskQuery = 'SELECT status FROM adverts WHERE username = ? AND DATE(last_updated) = CURDATE()';
    ...
    if (result.affectedRows === 0) {
      return res.status(400).json({ error: 'Task update failed. Either the task is already done or you do not have a pending task.' });
    }
...
app.get('/get-username', (req, res) => {
  const username = req.session.username;
  const getUserInfoQuery = 'SELECT username, account_status, current_account, ballance, phone_number, email, inviter_link FROM new_transcribers WHERE username = ?';
...
app.get('/get-user-invites', (req, res) => {
  ...
  const getUserInfoQuery = 'SELECT invited, inviter, firstrecharge, earned FROM invites WHERE inviter = ?';

  pool.promise().execute(getUserInfoQuery, [inviter])
    .then(([results, fields]) => {
      if (results.length > 0) {
        // Map the results to an array of transactions
